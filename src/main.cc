...
template <typename T> inline void print_vector(vector<T> v) {
  std::cout << "[ ";
  for_each(v.begin(), v.end(), [](T i) { cout << i << " "; });
  std::cout << "]" << std::endl;
}

vector<int> get_order(bool file_flag, string file_name) {
...
void parse_command_line_arguments(int argc, char *argv[], bool *file_flag,
                                  string *file_name) {
...
int main(int argc, char *argv[]) {
  ...
  vector<int> passenger_seat_order = get_order(file_flag, file_name);

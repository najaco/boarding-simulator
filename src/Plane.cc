...
unsigned int Plane::board() {
  ...
  line[this->passengers.size()].lock(); // Lock position in array so no passenger can begin movement
  ...
  for (auto &th : passengers_threads)
    th.join();
  // Must wait for threads to finish before continuing
  if (this->verbose >= 1) { // If any messages were used, must print new line character
    cout << endl;
  }
...
void Plane::go_to_seat(Passenger p) {
  ...
  unique_lock<mutex> lk(board_mutex); // unique lock that is shared across all threads
  ...
  while (p.position != p.seat_number + this->passengers.size()) {
    ...
    this_thread::sleep_for(chrono::nanoseconds(STEP_TIME)); // sleep to force a context switch
    p.position++;
  }
